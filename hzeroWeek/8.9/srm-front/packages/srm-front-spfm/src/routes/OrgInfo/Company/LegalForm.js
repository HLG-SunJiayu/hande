...
import { isUndefined, find, isNumber } from 'lodash';
...
export default class LegalForm extends PureComponent {
  ...
  render() {
    ...
    if (licenceUrl && this.uploadButton) {
      if (licenceUrl && this.uploadButton) {
        const url = getAttachmentUrl(licenceUrl, 'hpfm-comp', getCurrentOrganizationId());
        fileList.push({
          uid: url,
                  ...
                  rules: [
                    {
                      ...
                      message: intl.get('hzero.common.validation.requireSelect', {
                        name: intl.get('spfm.enterprise.view.message.licenceEndDate'),
                      }),
                    },
                  ],
                })(
