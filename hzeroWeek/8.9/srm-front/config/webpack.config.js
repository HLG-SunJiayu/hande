...
module.exports = function(webpackEnv) {
  ...
  const hzeroFrontChildNodeModulePathReg = new RegExp(
    `(srm|hzero)-front\\S*\\${path.sep}node_modules`);
  // hzero-front 子项目代码路径
  const hzeroFrontBuildLibPathReg = new RegExp(
    `(srm|hzero)-front\\S*\\${path.sep}lib`);

  // common function to get style loaders
  const getStyleLoaders = (cssOptions, preProcessor, others) => {
    const loaders = [
        ...
        options: {
          ...
          getLocalIdent: (context, localIdentName, localName) => {
            if (
              ...
              const depIndex = Math.max(context.resourcePath.lastIndexOf(
                'node_modules'), context.resourcePath.lastIndexOf(
                'hzero-front')) - 2;
              const depPIndex = context.resourcePath.lastIndexOf(path.sep,
                depIndex);
              const depP = context.resourcePath.substring(depPIndex,
                depIndex + 1);
              if (/^\/hzero-front/.test(depP)) {
                return localName;
              }
            }
            const match = context.resourcePath.match(
              new RegExp(`(src|(srm|hzero)-front\\S*\\${path.sep}lib)(.*)`));
            if (match && match[0] && !context.resourcePath.includes(`node_modules${path.sep}codemirror`)) {
              const antdProPath = match[0].replace('.less', '');
              const arr = antdProPath.split(path.sep).
                map(a => a.replace(/([A-Z])/g, '-$1')).
                map(a => a.toLowerCase());
              return `${env.raw.APP_NAME}-${arr.join(
                '-')}-${localName}`.replace(/--/g, '-');
            }
            return localName;
          },
  ...
  return {
    ...
    plugins: [
      ...
      new WebpackBar({ name: '🚚  SRM Front ', color: '#29BECE' }),
