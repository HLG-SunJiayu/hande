...
export default class Delivery extends React.Component {
  ...
  onOk() {
    ...
    if (!isEmpty(deliveryKeys)) {
      ...
      message.warning(
        intl.get(`${promptCode}.view.message.confirm.selected.field`).d('请选择要显示的送货条目！')
      );
    }
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${promptCode}.model.invoiceBill.processUser`).d('送货条目'),
      ...
      <div className={styles.delivery}>
        <Row className={styles['card-row']}>
          <div className={styles['card-img']}>
            <span className={styles['card-title']}>送货</span>
        ...
        <Modal
          title={intl.get('hzero.common.button.operating').d('选择需要展示的送货条目')}
