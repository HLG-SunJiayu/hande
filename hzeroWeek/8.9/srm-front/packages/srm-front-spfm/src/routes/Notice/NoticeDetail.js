...
import { Button, Form, Input, Row, Col, DatePicker, Select } from 'hzero-ui';
...
export default class NoticeDetail extends React.PureComponent {
      ...
      this.queryNoticeDetail({ noticeId }).then(res => {
        if (res && res.attachmentUuid) {
          ...
          dispatch({
            ...
            payload: { attachmentUUID: res.attachmentUuid, bucketName: 'public' },
          });
        }
      });
    } else {
  ...
  renderForm() {
    const {
      form,
      notice: { noticeType = [], langList = [], noticeDetail = {} },
    } = this.props;
