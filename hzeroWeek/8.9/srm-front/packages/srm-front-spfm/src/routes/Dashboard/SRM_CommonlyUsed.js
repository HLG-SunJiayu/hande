...
@connect(({ srmCards, loading }) => ({
  srmCards,
  allLoading: loading.effects['srmCards/queryAllFunctions'],
  addLoading: loading.effects['srmCards/addFunctions'],
}))
...
export default class CommonlyUsed extends React.Component {
  ...
  handleSearch() {
    ...
    dispatch({
      type: 'srmCards/queryFunctions',
    }).then(res => {
      if (res) {
        dispatch({
          type: 'srmCards/updateState',
          payload: { commonlyUsedLoading: false },
        });
      }
    });
  }
  ...
  handleAllSearch() {
    const { dispatch } = this.props;
    dispatch({
      type: 'srmCards/queryAllFunctions',
    });
  }
  ...
  @Bind()
  onOk() {
    const { dispatch, srmCards: { allFunction = [] } = {} } = this.props;
    ...
    if (!isEmpty(this.addFunctionList)) {
      if (this.addFunctionList.length > 7) {
        message.warning(
          intl
    ...
    return (
      <Row>
        <Row className={styles.commonlyUsed}>
          <div>
            <span className={styles['card-title']}>常用功能</span>
