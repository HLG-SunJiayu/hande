...
export default class MergeRuleModal extends Component {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get(`${promptCode}.mergeCompanyFlag`).d('企业'),
      ...
      {
        title: intl.get(`${promptCode}.supplierSiteFlag`).d('供应商地点'),
      ...
      {
        title: intl.get(`${promptCode}.inventoryFlag`).d('收货库房'),
      ...
      {
        title: intl.get(`${promptCode}.shipToThirdPartyFlag`).d('送达方'),
      ...
      {
        title: intl.get(`${promptCode}.sourcePlatform`).d('订单来源'),
      ...
      {
        title: intl.get(`${promptCode}.immedShippedFlag`).d('直发属性'),
    ...
    return (
      <Modal
        title={intl.get(`${promptCode}.mergeRule.title`).d('并单规则定义')}
        visible={visible}
        footer={null}
        width={1200}
